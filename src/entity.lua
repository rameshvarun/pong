...
function Entity:addComponent(comp)
  ...
  comp:start() -- Invoke the start callback on this component.
...
function Entity:includeComponent(comp)
  ...
  for key, value in pairs(comp.class.__instanceDict) do
    if type(value) == "function" and not key:startsWith("__") and
      ...
      key ~= "debugOverlay" and key ~= "update" and key ~= "initialize" and key ~= "destroy" then
        self[key] = function(_, ...)
...
-- Mark this entity for removal, and invoke destroy on all of the components.
